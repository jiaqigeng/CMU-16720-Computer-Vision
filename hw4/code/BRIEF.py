...
def makeTestPattern(patch_width=9, nbits=256):
    ...
    test_pairs = np.random.normal(loc=0, scale=0.2 * patch_width, size=(nbits, 2, 2,))
    test_pairs[test_pairs < -(patch_width // 2)] = -(patch_width // 2)
    test_pairs[test_pairs > (patch_width // 2)] = patch_width // 2
    test_pairs = np.round(test_pairs).astype(np.int)
    compareX, compareY = test_pairs[:, 0, :], test_pairs[:, 1, :]
    compareX = compareX[:, 1] * patch_width + compareX[:, 0]
    compareY = compareY[:, 1] * patch_width + compareY[:, 0]
...
def computeBrief(im, gaussian_pyramid, locsDoG, k, levels,
    ...
    locs = locsDoG[np.where((locsDoG[:, 0] + patch_width // 2) < w)]
    locs = locs[np.where((locs[:, 0] - (patch_width // 2)) >= 0)]
    locs = locs[np.where((locs[:, 1] + patch_width // 2) < h)]
    locs = locs[np.where((locs[:, 1] - (patch_width // 2)) >= 0)]

    gaussian_pyramid_roll = np.rollaxis(gaussian_pyramid, 2, 0)

    desc = np.zeros((locs.shape[0], compareX.shape[0]))

    for i in range(locs.shape[0]):
        x, y, l = locs[i, 0], locs[i, 1], locs[i, 2]
        x_min = x - (patch_width // 2)
        y_min = y - (patch_width // 2)
        x_max = x + (patch_width // 2)
        y_max = y + (patch_width // 2)

        target_area = gaussian_pyramid_roll[0][y_min:y_max + 1, x_min:x_max + 1].flatten()
        pt_x = target_area[compareX]
        pt_y = target_area[compareY]

        bits = np.zeros(compareX.shape)

        bits[np.where(pt_x < pt_y)[0]] = 1
        desc[i, :] = bits
