...
for k, v in params.items():
    ...
    ##########################
        if k == 'cache_layer1':
            ...
            for i in range(params["Wlayer1"].shape[0]):
                for j in range(params["Wlayer1"].shape[1]):
            ...
            for i in range(params["blayer1"].shape[0]):
                params["blayer1"][i] = params_orig["blayer1"][i] + eps
                h1 = forward(X, params, 'layer1')
                probs = forward(h1, params, 'output', softmax)
                loss1 = -np.sum(y * np.log(probs))
        ...
        if k == 'cache_output':
            X, pre_act, post_act = v
            for i in range(params["Woutput"].shape[0]):
                for j in range(params["Woutput"].shape[1]):
            ...
            for i in range(params["boutput"].shape[0]):
                params["boutput"][i] = params_orig["boutput"][i] + eps
                probs = forward(X, params, 'output', softmax)
                loss1 = -np.sum(y * np.log(probs))
